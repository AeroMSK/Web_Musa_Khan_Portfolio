class LetterGlitch {
    constructor(canvasId, options = {}) {
        ...
        // Options with defaults matches React component
    ...
    resizeCanvas() {
        const parent = this.canvas.parentElement; // Usually body or a wrapper
        if (!parent) return;

        const dpr = window.devicePixelRatio || 1;
        // If attached to body/fixed, use window dimensions or parent dimensions
        const rect = parent.getBoundingClientRect();
        // Fallback to window inner size if parent rect is small (e.g. 0 height)
    ...
    drawLetters() {
        ...
        // Use styled width/height for clearRect calculation
        ...
        // Draw vignette background if needed (though CSS is preferred for static gradients)
        // Note: The React component used CSS/divs for Vignette. We'll handle Vignette in CSS or here if requested.
        // For performance, doing it in CSS is better, but here's how to do in canvas if needed:
        // However, the text is drawn OVER the background.
